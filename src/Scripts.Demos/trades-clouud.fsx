...
let meanPriceOnDay id day =
  let ts = BigDeedle.Trades.GetFrame(id)?Price
...
/// Calculate returns for all days in a given year and month. Here, 
/// we can nicely use MBrace's "Cloud.Parallel" and "local" workflow to scale 
...
let returnsForMonth y m = cloud {
  ...
  let! returns = 
    [| for d in 1 .. lastDay -> local {
        ...
        return DateTime(y,m,d), res } |] |> Cloud.Parallel
