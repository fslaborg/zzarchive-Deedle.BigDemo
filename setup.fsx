...
/// Creates an atom that's used for progress reporting
let log = CloudAtom.New "" |> cluster.Run
...
let downloadPrices id url = cloud { 
  ...
  let writeBatch index (batch:list<string>) = cloud { 
    ...
    do! CloudAtom.Update(log, fun _  -> "Last line: " + batch.Head) }
...
CloudAtom.Read log |> cluster.Run |> printfn "%s"
...
w1.Completed
w1.CancellationTokenSource.Cancel()
w1.AwaitResult()
w2.Completed
